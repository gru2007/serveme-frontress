...
describe BuildCaddyCloudflareImageWorker do
  ...
  it "builds with --pull and pushes :latest to Docker Hub" do
    expect(Open3).to receive(:capture2e).with("docker", "build", "--pull", "-t", "serveme/caddy-cloudflare:latest", described_class::DOCKER_DIR).and_return([ "", ok ])
    expect(Open3).to receive(:capture2e).with("docker", "push", "serveme/caddy-cloudflare:latest").and_return([ "", ok ])
    worker.perform
  end
