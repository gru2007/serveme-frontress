...
describe LogWorker do
  ...
  let(:server)                { double(:server, id: 1, rcon_auth: true, condenser: condenser, supports_mitigations?: false, provision_estimate: nil, end_estimate: nil, cloud?: false).as_null_object }
  ...
  describe 'connects' do
    ...
    it 'kicks SDR connections without normal IP history and enqueues profile check' do
      ...
      ReservationPlayer.should_receive(:sdr_eligible_steam_profile?).with(76561197960365727).and_return(false)
