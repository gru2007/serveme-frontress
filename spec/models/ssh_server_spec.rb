...
describe SshServer do
  describe '#remove_configuration' do
    it 'deletes the reservation configs' do
      ...
      subject.should_receive(:execute).with("rm -f /tmp/foo/tf/cfg/reservation.cfg /tmp/foo/tf/cfg/#{Frontress::DEFAULT_MAP}.cfg /tmp/foo/tf/cfg/banned_user.cfg /tmp/foo/tf/cfg/banned_ip.cfg /tmp/foo/tf/motd.txt")
  ...
  describe '#find_process_id' do
    it 'finds the pid of the game server, not of the wrappers around it' do
      subject.stub(port: '27015')
      subject.should_receive(:execute).with("ps ux | grep port | grep #{subject.port} | grep #{Frontress::SERVER_PROCESS} | grep -v srcds_run_64 | grep -v steam-runtime-tools | grep -v grep | grep -v ruby | awk '{print $2}'")
